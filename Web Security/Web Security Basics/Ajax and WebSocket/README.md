...
Another way to talk to the server is called a WebSocket. The Ajax has a limitation, because the server cannot push data to you. When the Ajax sends a request, gets the response, the connection is broken, and the client needs to initiate the communication everytime. When there's some change on the server, they want push the data to the client without the client asking for an update. So, this led to the development of the WebSocket to satisfy this kind of need. The WebSocket is similar to a normal socket, the TCP socket. But inside side of it, they run the HTTP protocol. So the connection is foing to be persistent to a certain degree. When the server has the update, they send the data to the client.
So we want to point out the security policy in the WebSocket which the fundamental difference between Ajax and WebSocket. In Ajax, you do have the same-origin policy inforced while WebSocket does not have it. For the WebSocket, the browser will not enforce the policy. In fact, the developer on the server side have the responsibility to put that in your code. If you don't want the data to go though cross-origin request, you can check for that and make a decision to send the data back or not. The Ajax is different because whe you send the data the browser inforces the policy.
